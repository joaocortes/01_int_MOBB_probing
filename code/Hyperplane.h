...
public:
	 ...
	 * \return true if the point is dominated by the hyperplane.
	 */
	bool dominates_eps(std::vector<int>& y, double eps);
	...
	*/
	double computeGapEps(LocalUpperBound* u, Statistics* stat);

	/* Compute the gap between hpp and lub u on objective k
	*/
	double computeGapEps(LocalUpperBound* u, int k, Parameters* P);

	/* Compute the eps dominance val between the hyperplane and the lub u.
	*/
	double computeGapRhs(LocalUpperBound* u, Statistics* stat, Parameters* P);
