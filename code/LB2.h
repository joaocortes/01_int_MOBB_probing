...
public:
	 ...
	 */
	LowerBoundSet(MathematicalModel* lp, BranchingDecisions* branch, Parameters* P);
	 ...
	 */
	virtual bool gatherIntegerSolutions(UpperBoundSet& U);
...
private:
	...
	bool reset; //!< used if manually reset
...
public:
	...
	*
	...
	int computeMostOftenFractionalSplittingValueVF(SLUB& slub, int i, int minLb, int maxUb);
	...
	*/
	 ...
	 */
	void extractCoverCuts(BranchingDecisions* bd, MathematicalModel* pb);

	double getPercentageViolation(BranchingDecisions* bd);
	...
	std::list<Hyperplane*>* getListFacets();
};