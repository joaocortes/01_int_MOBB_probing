...
/* ==========================================================
  ...
  */
UpperBoundSet::UpperBoundSet(MathematicalModel& lp) : incumbentSet(0), lastGeneratedId(0), lp(&lp), cpuSol(), A() {
    lubSet = std::list<LocalUpperBound>(0);
    lubSet.push_back(LocalUpperBound(lp));
}

UpperBoundSet::~UpperBoundSet() {

    for (std::list<Solution*>::iterator y = incumbentSet.begin(); y != incumbentSet.end(); y++) {
        delete* y;
    }
    incumbentSet.clear();
}
 ...
 */
bool UpperBoundSet::updateUB(Point& P) { // void
...
void UpperBoundSet::updateUB(BranchingDecisions* bd) {
    ...
    nonDominated = newSol->isFeasible(lp, bd); // note: nonDominated has the same role as feasible
...
void UpperBoundSet::updateLUB(Solution* s) {
    ...
    std::list<LocalUpperBound*> Atemp; // list of lub that should be discarded because strictly dominated by s
    ...
    for (uDiscarded = Atemp.begin(); uDiscarded != Atemp.end(); uDiscarded++) {
...
/* return the value of wasUpdated, and reset it to its default value false.
 */
bool UpperBoundSet::wasUpdated() {
    bool sv = flag;
    flag = false;
    return sv;
}

/* check whether the lub at the address u is discarded or not (loop trough A)
 */
bool UpperBoundSet::checkDiscardedLubs(LocalUpperBound* u) {

    bool exists = false;
    std::list<LocalUpperBound*>::iterator uDisc = A.begin();
    while (!exists && uDisc != A.end()) {
        if (*uDisc == u) exists = true;
        uDisc++;
    }
...
/* Clear out the discarded Lubs (stored in A) after the global gap was updated
 */
void UpperBoundSet::clearDiscardedLubs() {
    A.clear();
}