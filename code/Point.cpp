...
/* Generate cover cuts given the branching decisions
 *
 * \param bd, BranchingDecisions.
 */
void Point::generateCoverCuts(BranchingDecisions* bd, MathematicalModel* lp, int iteration) {

    int p = objVector.size();
    std::vector<int> threshold(bd->slub);
    std::vector<bool> finite(lp->get_p(), false);
    std::list<int> candidates(0);
    CoverCut baseCoverCut;
    std::list<CoverCut> generatedCuts;
    bool addedToCut;


    if (!is_ray()) { // generate cover cut only if it is an extreme point...

        // compute candidates list & base cover cut

        baseCoverCut = CoverCut(lp->get_p());
        for (int k = 0; k < lp->get_p(); k++) {
            if (bd->slub[k] <= 1000000) finite[k] = true;
            baseCoverCut.threshold[k] = bd->slub[k];
        }

        for (int i = 0; i < preImage.size(); i++) {
            if (preImage[i] >= 1 - EPS_INT) { // variables set to 1 in the lp solution are considered to be fixed
                for (int k = 0; k < lp->get_p(); k++) {
                    threshold[k] -= lp->get_objective(k, i);
                    baseCoverCut.threshold[k] -= lp->get_objective(k, i);
                    baseCoverCut.coefs[k].push_back(i);
                }
                baseCoverCut.idx.push_back(i);
                baseCoverCut.rhs++;
            }
            else if (preImage[i] >= 0 + EPS_INT) { // variables fractional in the lp solution are added to candidate list
        ...
        bool violateThreshold;
        for (std::list<int>::iterator c = candidates.begin(); c != candidates.end(); c++) {

            addedToCut = false;

            // case where we can add the var to an existing cover cut, i.e. cut is lifted

            for (std::list<CoverCut>::iterator cut = generatedCuts.begin(); cut != generatedCuts.end(); cut++) {

                violateThreshold = false;
                for (int k = 0; k < lp->get_p(); k++) violateThreshold = violateThreshold || (lp->get_objective(k, *c) > cut->threshold[k] && lp->get_objDir(k) == -1);

                if (violateThreshold) {
                    cut->idx.push_back(*c);

                    for (int k = 0; k < lp->get_p(); k++) {
                        cut->coefs[k].push_back(lp->get_objective(k, *c));
                        // search for highest cost to update the internal threshold
                        int max = -1000000;
                        for (std::list<int>::iterator cf = cut->coefs[k].begin(); cf != cut->coefs[k].end(); cf++) {
                            if (*cf > max) max = *cf;
                        }
                        cut->threshold[k] += max - lp->get_objective(k, *c);
                        addedToCut = true;
                    }
                }
            }

            // if cannot be used to lift an existing cut, we create a new one for this variable if it violates the threshold

            violateThreshold = false;
            for (int k = 0; k < lp->get_p(); k++) violateThreshold = violateThreshold || (lp->get_objective(k, *c) > baseCoverCut.threshold[k] && lp->get_objDir(k) == -1);

            if (!addedToCut && violateThreshold) {
                generatedCuts.push_back(CoverCut(baseCoverCut));
                generatedCuts.back().idx.push_back(*c);
                generatedCuts.back().rhs++;
                for (int k = 0; k < lp->get_p(); k++) {
                    generatedCuts.back().coefs[k].push_back(lp->get_objective(k, *c));
                    // search for highest cost to update the internal threshold
                    int max = -1000000;
                    for (std::list<int>::iterator cf = generatedCuts.back().coefs[k].begin(); cf != generatedCuts.back().coefs[k].end(); cf++) {
                        if (*cf > max) max = *cf;
                    }
                    generatedCuts.back().threshold[k] += max - lp->get_objective(k, *c);
                }
            }
        }

        // add cuts to the branching decisions

        for (std::list<CoverCut>::iterator cut = generatedCuts.begin(); cut != generatedCuts.end(); cut++) {
            // print
            //if (iteration == 3689) {
            //    for (std::list<int>::iterator v = cut->idx.begin(); v != cut->idx.end(); v++) {
            //        std::cout << "+ x[" << *v << "] ";
            //    }
            //    std::cout << " <= " << cut->rhs << "\n";
            //}
            ...
            cut->coefs.clear();
            cut->threshold.clear();
            //bd->cc.push_back(CoverCut(*cut));

            double violation = 0;
            for (std::list<int>::iterator v = cut->idx.begin(); v != cut->idx.end(); v++) {
                violation += preImage[*v];
            }
            violation -= cut->rhs;
            if (violation >= 0.8) {
                //std::cout << " new cut violating by : " << violation << "\n";
                bd->cc.push_back(CoverCut(*cut));
            }
        }
        ...
        /*
        for (int k = 0; k < p; k++) {
            if (bd->slub[k] < 1000000) { // ... and objective function k has a OB constraint
                
                // reset the lists
                candidates.clear(); // fractional var
                generatedCuts.clear();
                

                // compute the threshold & the candidates list
                threshold = bd->slub[k];
                for (int i = 0; i < preImage.size(); i++) {
                    if (preImage[i] >= 1 - EPS_INT) { // variables set to 1 in the lp solution are considered to be fixed
                        threshold -= lp->get_objective(k, i);
                        baseCoverCut.idx.push_back(i);
                        baseCoverCut.coefs.push_back(i);
                        baseCoverCut.rhs++;
                        //if (iteration == 3689) std::cout << " -> " << i << " is basic\n";
                    }
                    else if (preImage[i] >= 0 + EPS_INT) { // variables fractional in the lp solution are added to candidate list
                        candidates.push_back(i);
                        //if (iteration == 3689) std::cout << " -> " << i << " is candidate\n";
                    }
                    else {
                        //if (iteration == 3689) std::cout << " -> " << i << " is not used\n";
                    }
                }
                baseCoverCut.threshold = threshold;
                ...
                for (std::list<int>::iterator c = candidates.begin(); c != candidates.end(); c++) {
                    ...
                    // case where we can add the var to an existing cover cut, i.e. cut is lifted
                    for (std::list<CoverCut>::iterator cut = generatedCuts.begin(); cut != generatedCuts.end(); cut++) {
                        if (lp->get_objective(k, *c) > threshold) {
                            cut->idx.push_back(*c);
                            cut->coefs.push_back(lp->get_objective(k, *c));
                            //cut->rhs++;
                            // search for highest cost to update the internal threshold
                            int max = -1000000;
                            for (std::list<int>::iterator cf = cut->coefs.begin(); cf != cut->coefs.end(); cf++) {
                                if (*cf > max) max = *cf;
                            }
                            cut->threshold += max - lp->get_objective(k, *c);
                            addedToCut = true;
                        }
                    }

                    // if cannot be used to lift an existing cut, we create a new one for this variable if it violates the threshold
                    if (!addedToCut && lp->get_objective(k, *c) > threshold) {
                        generatedCuts.push_back(CoverCut(baseCoverCut));
                        generatedCuts.back().idx.push_back(*c);
                        generatedCuts.back().coefs.push_back(lp->get_objective(k, *c));
                        generatedCuts.back().rhs++;
                        // search for highest cost to update the internal threshold
                        int max = -1000000;
                        for (std::list<int>::iterator cf = generatedCuts.back().coefs.begin(); cf != generatedCuts.back().coefs.end(); cf++) {
                            if (*cf > max) max = *cf;
                        }
                        generatedCuts.back().threshold += max - lp->get_objective(k, *c);
                    }
                }

                // add the cuts to the branching decisions
                for (std::list<CoverCut>::iterator cut = generatedCuts.begin(); cut != generatedCuts.end(); cut++) {
                    // print
                    if (iteration == 3689) {
                        for (std::list<int>::iterator v = cut->idx.begin(); v != cut->idx.end(); v++) {
...
/* Generate lifted cover cuts given the branching decisions (Balas lefting procedure)
 *
 * \param bd, BranchingDecisions.
 * \param lp, MathemeticalModel.
 */
void Point::generateLCI(BranchingDecisions* bd, MathematicalModel* lp, int iteration) {

    int p = lp->get_p();
    int n = lp->get_n();
    int adjustedRhs(0), threshold(0);
    int idx(0), maxViolation(0);
    int Cplus(0);
    bool isViolated = false;
    CoverCut cut;
    std::priority_queue<std::pair<double, int>> pq; // obj val -- idx
    std::list<std::pair<double, int>> tmp; // obj val -- idx
    ...
    if (!is_ray()) {
        ...
        for (int k = 0; k < p; k++) {
            ...
            cut = CoverCut(n);
            isViolated = false;
            pq = std::priority_queue<std::pair<double, int>>();
            tmp.clear();

            // compute the adjusted rhs (substract obj val of var fixed to 1 to rhs)

            adjustedRhs = bd->slub[k] - 1;
            for (int i = 0; i < n; i++) {
                if (bd->lb[i] == 1) adjustedRhs -= lp->get_objective(k, i);
            }

            // set all free variables with a coefficient that exceed the adjusted rhs to 0, since they cannot take value 1

            for (int i = 0; i < n; i++) {
                if (bd->lb[i] != bd->ub[i] && lp->get_objective(k, i) > adjustedRhs) {
                    bd->ub[i] = 0;
                    //std::cout << " -> x[" << i << "] to 0\n";
                }
            }
            ...
            // initiate a cut with all free variables taking value 1

            threshold = adjustedRhs;
            for (int i = 0; i < n; i++) {
                if (bd->lb[i] != bd->ub[i] && preImage[i] == 1) { // free variable taking value one
                    threshold -= lp->get_objective(k, i);
                    cut.rhs++;
                    cut.idxx[i] = 1;
                    pq.push(std::pair<int,int>(lp->get_objective(k, i), i));
                    //std::cout << "new pair pushed: " << lp->get_objective(k, i) << " , " << i << "\n";
                }
            }


            // case where the threshold is violated, i.e. point is located on the OB constraint

            if (threshold < 0) {
                //cut.rhs = cut.idx.size() - 1;
                isViolated = true;
            }

            // case where we explore fractional variables

            else {

                // search for the fractional variable that will complete the cover cut

                maxViolation = 0;
                for (int i = 0; i < n; i++) {
                    if (bd->lb[i] != bd->ub[i] && preImage[i] < 1 && preImage[i] > 0) { // free variable not taking value 1 nor 0, i.e. is fractional
                        if (lp->get_objective(k, i) > threshold) {
                            if (preImage[i] > maxViolation) {
                                idx = i;
                                maxViolation = preImage[i];
                            }
                            isViolated = true;
                        }
                    }
                ...
                cut.rhs++;
                cut.idxx[idx] = 1;
                pq.push(std::pair<int, int>(lp->get_objective(k, idx), idx));
            }


            // if a possible violating cut was detected
            ...
            if (isViolated) {
                ...
                double sigma = -adjustedRhs;
                for (int i = 0; i < n; i++) if (cut.idxx[i] == 1) sigma += lp->get_objective(k, i);

                double aBar = pq.top().first;
                tmp.push_back(pq.top());
                pq.pop();

                while (!pq.empty()) {
                    if (!breaked) {
                        delta = aBar - pq.top().first;
                        if (l * delta < sigma) {
                            aBar = pq.top().first;
                            sigma = sigma - l * delta;
                        }
                ...
                if (sigma > 0) {
                    aBar = double(adjustedRhs) / double(cut.rhs + 1);
                }


                // compute new a_j & S, and reset coef of varibales in C+

                while (!tmp.empty()) {
                    if (tmp.front().first > aBar) {
                        tmp.front().first = aBar;
                        cut.idxx[tmp.front().second] = 0;
                        Cplus++;
                    }
                    tmp.front().first = min(tmp.front().first, aBar);
                    pq.push(tmp.front());
                    tmp.pop_front();
                }
                ...
                l = 0;
                while (!pq.empty()) {
                    S.push_back(S[l] + pq.top().first);
                    //std::cout << " S[" << l + 1 << "] = " << S[l] << " + " << pq.top().first << "\n";
                    pq.pop();
                    l++;
                }


                // lift free variables with 0 coefficient in the cut

                int h = 0;
                for (int i = 0; i < n; i++) {
                    if (bd->lb[i] != bd->ub[i] && cut.idxx[i] != 1 && lp->get_objective(k, i) != 0) {

                        // find the appropriate S
                        h = 0;
                        while (S[h] <= lp->get_objective(k, i)) {
                            h++;
                        }
                        h--;

                        // set the value of the coef in the cut, using superadditive functions
                        bool cond1 = false;
                        for (int g = 0; g < Cplus - 1; g++) {
                            if (lp->get_objective(k, i) == g * aBar) cond1 = true;
                        }
                ...
                // add the cut to the branchingDecisions
                ...
                // print the cut
                //for (int i = 0; i < n; i++) {
                    //if (cut.idxx[i] != 0) std::cout << " + " << cut.idxx[i] << " x[" << i << "]";
                //}
                //std::cout << " <= " << cut.rhs << "\n";
