...
double FeasibilityCheckModel::extractConstant(MathematicalModel& LP, BranchingDecisions* branchDec) { // , BranchingDecisions* branchDec
	...
	// point-based cover cuts

	if (coverCuts.getSize() == branchDec->cc.size()) {
		int i = 0;
		for (std::list<CoverCut>::iterator cut = branchDec->cc.begin(); cut != branchDec->cc.end(); cut++) {
			theRightHandSide += cut->rhs * cplex.getDual(coverCuts[i]);
			i++;
		}
	}
...
void FeasibilityCheckModel::applyCoverCuts(BranchingDecisions* bd) {

	model.remove(coverCuts);
	coverCuts.clear();

	for (std::list<CoverCut>::iterator cut = bd->cc.begin(); cut != bd->cc.end(); cut++) {
		coverCuts.add(IloRange(env, -IloInfinity, cut->rhs));
		//for (std::list<int>::iterator v = cut->idx.begin(); v != cut->idx.end(); v++) {
			//coverCuts[coverCuts.getSize() - 1].setLinearCoef(x[*v], 1);
		//}
		for (int i = 0; i < n - 1; i++) {
			coverCuts[coverCuts.getSize() - 1].setLinearCoef(x[i], cut->idxx[i]);
		}
	}
...
/*! \brief Solve the Weighted Sum model.
	 ...
	 * \param branching decisions bd -> current node subpb
	 * \param int index -> index of the variable being tested
	 * \param int dir -> direction of optimization (1 for min, -1  for max)
	 * \return true if it is feasible and solved to optimality, false otherwise [ToDo]
	 */
bool ProbingModel::solveInt(BranchingDecisions& bd, int index, int dir) {

	bool solved = false;
	resetBounds(bd);
	resetObjInt(index, dir);
...
void ProbingModel::resetObjInt(int index, int dir) {
	for (int i = 0; i < n; i++) {
		ptrObj.setLinearCoef(x[i], 0);
	}
	ptrObj.setLinearCoef(x[index], dir);
}
...
void ProbingModel::applyCoverCuts(BranchingDecisions* bd) {

	model.remove(coverCuts);
	coverCuts.clear();

	for (std::list<CoverCut>::iterator cut = bd->cc.begin(); cut != bd->cc.end(); cut++) {
		//std::cout << " new cut with rhs " << cut->rhs << "\n";
		coverCuts.add(IloRange(env, -IloInfinity, cut->rhs));
		/*for (std::list<int>::iterator v = cut->idx.begin(); v != cut->idx.end(); v++) {
			coverCuts[coverCuts.getSize() - 1].setLinearCoef(x[*v], 1);
		}*/
...
void ProbingModel::extractModel() {
	cplex.extract(model);
	cplex.exportModel("solKP.lp");
}
...
void VariableFixingModel::build(MathematicalModel& LP, Parameters* P) {
	...
	IloExpr obj(env);
	for (int i = 0; i < LP.get_n(); i++) {
		obj += 0 * x[i]; // to be modified before solving   LP.get_objective(0, i)
	}
	...
	for (int i = 0; i < LP.get_p(); i++) {
		ptrCtesOB.add(IloRange(env, -IloInfinity, IloInfinity)); // change rhs (p) 0  IloInfinity
		for (int j = 0; j < LP.get_n(); j++) {
			ptrCtesOB[i].setLinearCoef(x[j], LP.get_objective(i, j));
		}
		model.add(ptrCtesOB[i]);
...
void VariableFixingModel::exportPresolve() {

	CPXCENVptr cenv = cplex.getImpl()->getCplexEnv();
	CPXLPptr clp2 = cplex.getImpl()->getCplexLp();
	int nbcols = cplex.getNcols();
	int nbrows = cplex.getNrows();
	...
	int pssuccess = CPXXpresolve(cenv, clp2, CPX_ALG_NONE);
	int statuss = CPXXgetprestat(cenv, clp2, &prestat_p, pcstat, 0, 0, 0);
	std::cout << "status: " << prestat_p << "\n";

	for (int i = 0; i < n; i++) {
		std::cout << "x" << i << " -> " << pcstat[i] << "\n";
	}
	int status = CPXXwriteprob(cenv, clp2, "preslv.lp", NULL);
	*/

	double* redlb = new double[nbcols];
	double* redub = new double[nbcols];
	int* rstat = new int[nbrows];

	int pssuccess = CPXXbasicpresolve(cenv,clp2,redlb,redub,rstat);
	if (pssuccess == 0) {
		for (int i = 0; i < n; i++) {
			std::cout << redlb[i] << " <= x[" << i << "] <= " << redub[i] << "\n";
		}
		std::cout << "\n";
		for (int j = 0; j < m; j++) {
			if (rstat[j] == -1) {
				std::cout << " -> constraint " << j << " is redundant\n";
			}
		}
	}
	else {
	...
	delete[] redlb, redub, rstat;



}

bool VariableFixingModel::extractPresolve(BranchingDecisions* bd) {
	...
	int nodeClosed = CPXXbasicpresolve(cenv, clp2, redlb, redub, rstat);
	if (!nodeClosed) {
		for (int i = 0; i < n; i++) {
			bd->lb[i] = redlb[i];
			bd->ub[i] = redub[i];
...
GapModel::GapModel() {
	IloNumVar t;
	x = IloNumVarArray(env);
	ptrCtesLine = IloRangeArray(env);
	ptrCtesOB = IloRangeArray(env);
}

void GapModel::build(MathematicalModel& LP, Parameters* param) {
	...
	IloExpr obj(env);
	obj = t;
	model.add(IloObjective(env, obj, IloObjective::Maximize));
	obj.end();

	// building intitial constraints
	...
	for (int i = 0; i < LP.get_p(); i++) {
		ptrCtesOB.add(IloRange(env, -IloInfinity, IloInfinity)); // change LB (u) -> Cx + te >= u
		for (int j = 0; j < LP.get_n(); j++) {
			ptrCtesOB[i].setLinearCoef(x[j], LP.get_objective(i, j));
		}
		model.add(ptrCtesOB[i]);
	...
	// building line constraints

	for (int k = 0; k < LP.get_p(); k++) {
		ptrCtesLine.add(IloRange(env, -IloInfinity, IloInfinity)); // change rhs (LB)
		for (int j = 0; j < LP.get_n(); j++) {
			ptrCtesLine[k].setLinearCoef(x[j], LP.get_objective(k, j));
		}
		ptrCtesLine[k].setLinearCoef(t, 1);
		model.add(ptrCtesLine);
	}

}

bool GapModel::solve(BranchingDecisions& bd) {
	return true;
}

void GapModel::resetBounds(BranchingDecisions& bd) {
...
double GapModel::retrieveObjectiveValue(){
	return 0.0;
}
 ...
 * This function creates an empty model.
 */
BoxEstimateModel::BoxEstimateModel() {
	x = IloNumVarArray(env);
	ptrObj = IloObjective(env);
};
 ...
 * \param LP MathematicalModel. This is the model of the initial problem, from which this linear program is built.
 */
void BoxEstimateModel::build(MathematicalModel& LP, Parameters* P) {
 ...
 * \param bd BranchingDecisions. The data structure that describes the branching decisions to apply.
 */
void BoxEstimateModel::resetBounds(BranchingDecisions& bd) {

};

void BoxEstimateModel::solve(int k, Parameters* P, MathematicalModel* lp) {
	...
	try
	{
		...
		if (solved) {
			...
			int valDbg = 0;
			for (int i = 0; i < lp->get_n(); i++) {
				valDbg += rlt[i] * lp->get_objective(k, i);
				//std::cout << rlt[i] << " * " << lp->get_objective(k, i) << " + ";
			}
			//std::cout << "\ncomputed opt val for " << k << ": " << valDbg << "\n";

			int val = 0;
			P->ideal[k] = cplex.getObjValue();
			for (int l = 0; l < lp->get_p(); l++) {
				if (l != k) {
					val = 0;
					for (int i = 0; i < lp->get_n(); i++) {
						val += rlt[i] * lp->get_objective(l, i);
					}
					if (val > P->nadir[l]) {
						P->nadir[l] = val;
					}
