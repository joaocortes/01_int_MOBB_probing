...
public:
	...
	void applyCoverCuts(BranchingDecisions* bd);
...
public:
	 ...
	 * \return true if it is feasible and solved to optimality, false otherwise [ToDo]
	 */
	bool solveInt(BranchingDecisions& bd, int index, int dir);
	void resetObjInt(int index, int dir);
...
public:
	...
	bool extractPresolve(BranchingDecisions* bd);
};
...
class GapModel : public CplexModel {
private:
	IloNumVar t;
	IloNumVarArray x; //!< variables from the initial problem
	IloRangeArray ptrCtesLine; //!< the objective function object, explicitely stored here to be modified prior to solving
	IloRangeArray ptrCtesOB; //!< a pointer to the constraints used for objective branching.
...
class BoxEstimateModel : public CplexModel {
private:
	IloNumVarArray x; //!< variables from the initial problem
	IloObjective ptrObj; //!< the objective function object, explicitely stored here to be modified prior to solving
	//IloRangeArray ptrCtesOB; //!< a pointer to the constraints used for objective branching.
...
public:
	...
	void solve(int k, Parameters* P, MathematicalModel* lp);
