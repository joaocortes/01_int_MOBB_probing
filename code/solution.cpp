...
Solution::Solution(BranchingDecisions* bd, MathematicalModel* lp) : dominated(false), cpuCreation(0) {
    ...
    //variableVector = std::vector<int>(n);
    for (int i = 0; i < n; i++) {
        variableVector.push_back(bd->lb[i]);
...
bool Solution::dominate(Solution& y) {
    ...
    do {
        ...
        // XE flag
        //else if (y.get_objVector(i) == objectiveVector[i]) {
            //nbEq++;
        //}
        i++;
    } while (domi && i < p);
    ...
    //if (nbEq == p) {
        //for (int i = 0; i < n; i++) {
            //if (variableVector[i] != y.get_preImage(i)) {
                //domi = false;
            //}
        //}
        //if (!domi) {
...
void Solution::print() {
    ...
    if (s2 >= 0) { // objectiveVector.size() - 1
        ...
        // pre-img
        if (variableVector.size() != 0) { //  && objectiveVector[2] <= -68
            std::cout << "\n   -> ( x[0] = ";
            for (int i = 0; i < variableVector.size() - 1; i++) {
                std::cout << variableVector[i] << " , x[" << i+1 << "] = ";
            }
...
bool Solution::isFeasible(MathematicalModel* lp, BranchingDecisions* bd) {
    ...
    for (int k = 0; k < objectiveVector.size(); k++) {
        if (feasible && objectiveVector[k] >= bd->slub[k]) {
            //if (feasible) std::cout << " out of range !\n";
            feasible = false;
            //print();
        }
    }
