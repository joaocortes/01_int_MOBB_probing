...
struct Statistics {
	Statistics() : solved(0), totalTime(), timeLBComputation(), timeDominanceTest(), timeUpdateUB(), timeComputeOB(), timeVarSel(), timeNodeSel(), cpuGapComputation(), nbNodes(0), nbFathomedInfeasibility(0), nbFathomedDominance(0), nbFathomedOptimality(0),
		minDepth(10000), maxDepth(0), avgDepth(0), lpSolved(0), timeUpdatePolyhedron(), timeFeasibilityCheck(), timeInitialization(), timeCopyLB(), avgExtrPts(0), avgFacets(0), avgVertices(0), maxFacets(0), maxVertices(0), minFacets(0), minVertices(0), nbNodesAtStage(0),
		...
		avgSubPbOB(0), minSubPbOB(100000), maxSubPbOB(0), nbOccurencesOB(0), avgNbLubs(0), maxNbLubs(0), nbDominanceTestLubHpp(0.0), nodesOBPerDepth(0), avgSubPbOBPerDepth(0), minSubPbOBPerDepth(0), maxSubPbOBPerDepth(0),
		gapNodeUpdated(1, 100), cpuGap(1, 0), gapNode(0), finalGap(0.0),
		nbSkippedLB(0), cpuSkip(),
		profiler(0), debug(), debugNbDomiTest(0), debugNbComputeGap(0) {} // , cpuSlubsOB(), cpuAdditionalDomiTestOB()
	...
	Timer cpuGapComputation; //!< cpu time spent in computing gaps
	...
	double avgExtrPts; //!< average number of extreme points (when feasible)
	...
	// gap measures
	std::vector<double> gapNodeUpdated; // gap updated with new nd pts at all nodes
	std::vector<double> cpuGap; // cpu at which gap is calculated
	std::vector<double> gapNode; // gap at creation of the node
	double finalGap; // final gap as absolute value
