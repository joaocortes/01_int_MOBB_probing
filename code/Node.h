...
private:
	...
	double gap; //!< gap at the current node
	std::vector<double> gapp; //!< gap at the current node per objective
	LocalUpperBound* gapingLub; //!< pointer to the lub with the largest gap in the node
	Hyperplane* gapingHpp; //!< pointer to the hpp of the LB set that defines the largest gap with gapingLub
...
public:
	 ...
	 */
	Node(MathematicalModel* lp, Parameters* par, Statistics* stat, UpperBoundSet* U, allCplexModels* acm);
	...
	void stopSol(BranchingDecisions* bd);
	void stopSol();
	...
	*/
	...
	bool fixAndSelectBranchingVariableInt(BranchingDecisions* newBd0, BranchingDecisions* newBd1, UpperBoundSet* U);
	...
	*/
	...
	/*! \brief Compute the rhs of the weighted sum scalarization when the node is created.
	 */
	double getWsScore(std::vector<double> l);
	void computeWsScore(std::vector<double> l);
	 ...
	 */
	void enumerate(BranchingDecisions* bd, int idx, int bound);
	void enumerate(BranchingDecisions* bd); // starting point of recursive enumeration

	/* Compute the gap given local LB & UB set
	*/
	void computeGap();
	void computeGap(LocalUpperBound* uMax, Hyperplane* hMax, BranchingDecisions* bd);
	void computeGap2();
	LocalUpperBound* getGapingLub();

	double getNbFreeVariables(); // binary only
	...
	std::list<Hyperplane*>* getLPrelaxPolyhedron();

	bool operator>(const Node& nd) const;

	int FLPBranch(BranchingDecisions* newBd);
};