 ...
 * \return true if the point is dominated by the hyperplane.
 */
bool Hyperplane::dominates_eps(std::vector<int>& y, double eps) {

    double val = 0;
    //int shiftLub = 10;
    for (int k = 0; k <= dim; k++) {
        val += (y[k]) * normalVector[k]; // - shiftLub 
    }

    double shift = eps * abs(rhs) / 100;
    //std::cout << "shit: " << shift << "\n";

    //double shift = 5.0;

    return val >= rhs + shift;
...
/* Compute the eps dominance val between the hyperplane and the lub u.
 */
double Hyperplane::computeGapEps(LocalUpperBound* u, Statistics* stat) {

    // get lub's coordinates

    std::vector<double> uu(dim + 1);
    for (int k = 0; k < dim + 1; k++) uu[k] = min(double(u->get_coordinate(k)), 100000.0);

    // get denominator

    //stat->debug.StartTimer();
    std::vector<double> e(dim + 1, 1);
    double denom = std::inner_product(normalVector.begin(), normalVector.end(), e.begin(), 0.0); // e.begin() P->selDir
    //stat->debug.StopTimer();

    // get eps and the new point

    double t = (std::inner_product(normalVector.begin(), normalVector.end(), uu.begin(), 0.0) - rhs) / denom;
    //std::cout << " t = " << t << "\n";

    // compute max proportion
    double prop = 0.0;
    //for (int k = 0; k < dim + 1; k++) prop = abs(max(100 - (100 * (uu[k] - t) / uu[k]), prop)); // abs ?
    for (int k = 0; k < dim + 1; k++) {
        if (uu[k] >= 0) prop = max(100 - (100 * (uu[k] - t) / uu[k]), prop); // abs ?
        else prop = max(100 - (100 * uu[k] / (uu[k] - t)), prop);
    }

    //return max(0.0, t); // gap is 0 if u located below this hpp
    return prop; // gap is 0 if u located below this hpp
}

/* Compute the gap between hpp and lub u on objective k
 */
double Hyperplane::computeGapEps(LocalUpperBound* u, int k, Parameters* P) {

    // get lub's coordinates

    std::vector<double> uu(dim + 1);
    for (int k = 0; k < dim + 1; k++) uu[k] = min(double(u->get_coordinate(k)), 100000.0); // lp max[k]
    ...
    if (normalVector[k] == 0) {
        ...
        v = (std::inner_product(normalVector.begin(), normalVector.end(), uu.begin(), 0.0) - rhs) / normalVector[k];
        prop = max(100 * v / (P->nadir[k] - P->ideal[k]), 0.0) ;
        //prop = 100 * max(v, 0.0) / uu[k]; // e.begin() P->selDir
    }
...
/* Compute the eps dominance val between the hyperplane and the lub u.
 */
double Hyperplane::computeGapRhs(LocalUpperBound* u, Statistics* stat, Parameters* P) {
    ...
    for (int k = 0; k <= dim; k++) {
        lhs += normalVector[k] * (u->get_coordinate(k) + P->shiftVec[k]); //P->GCD[k]
        shiftHpp += normalVector[k] * P->shiftVec[k];
    }

    stat->debug.StartTimer();
    double prop = max(100 * ((min(lhs,1000000.0) - (rhs + shiftHpp)) / (rhs + shiftHpp)), 0.0); // P->shift + 1
    stat->debug.StopTimer();

    //std::cout << "shift: " << shiftHpp << "\n";

    //std::cout << lhs << " vs " << rhs + shiftHpp << " --> " << prop << "%\n";
