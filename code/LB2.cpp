 ...
 */
LowerBoundSet::LowerBoundSet(MathematicalModel* lp, BranchingDecisions* branch, Parameters* P) : lp(lp), P(P), status(UNSOLVED), branchDec(branch), iteration(0) {}
 ...
 */
bool LowerBoundSet::gatherIntegerSolutions(UpperBoundSet& U) {
...
void LowerBoundSet::setStatus(int id) {
	status = id;
}
...
LinearRelaxation::LinearRelaxation(MathematicalModel* lp, WeightedSumModel* ws, FeasibilityCheckModel* feas, DualBensonModel* db, FurthestFeasiblePointModel* bvp, BranchingDecisions* branch, Statistics* S, Parameters* param) : LowerBoundSet(lp,branch,param), weightedSum(ws), feasibilityCheck(feas), dualBenson(db), furthestFeasiblePoint(bvp), antiIdealPoint(lp->get_p()), interiorPoint(lp->get_p()), boundingBox(0), warmstarted(false), firstGeneration(true), checkPointDestroyed(false), attemptedDeleteNewHpp(false), reset(false), facets(0), extrPoints(0), extrRays(0), nbIterations(0), call(0), S(S) { //stat()
    //initialize(*lp);
 ...
 * \param lp MathematicalModel*. A pointer to the problem this lower bound set refers to.
 */
LinearRelaxation::LinearRelaxation(MathematicalModel* lp, BranchingDecisions* branch, Statistics* S, Parameters* param) : LowerBoundSet(lp,branch,param), antiIdealPoint(lp->get_p()), interiorPoint(lp->get_p()), boundingBox(0), warmstarted(false), firstGeneration(true), checkPointDestroyed(false), attemptedDeleteNewHpp(false), reset(false), facets(0), extrPoints(0), extrRays(0), nbIterations(0), call(0), S(S) { // stat()
 ...
 */
LinearRelaxation::LinearRelaxation(LinearRelaxation* LPrelax, BranchingDecisions* branch, Parameters* param) : LowerBoundSet(LPrelax->lp,branch,param), weightedSum(LPrelax->weightedSum), feasibilityCheck(LPrelax->feasibilityCheck), dualBenson(LPrelax->dualBenson), furthestFeasiblePoint(LPrelax->furthestFeasiblePoint), antiIdealPoint(LPrelax->antiIdealPoint), interiorPoint(LPrelax->interiorPoint), warmstarted(true), firstGeneration(LPrelax->firstGeneration), checkPointDestroyed(LPrelax->checkPointDestroyed), attemptedDeleteNewHpp(LPrelax->attemptedDeleteNewHpp), reset(false), nbIterations(0), call(LPrelax->call), S(LPrelax->S) { // CHANGE WARMSTARTED TO TRUE LATER stat()
...
void LinearRelaxation::compute(UpperBoundSet* U, std::list<LocalUpperBound*> lubDomi) {
	...
	try {
		if (!isFeasible()) {
			...
			if (!warmstarted || attemptedDeleteNewHpp || reset) { // || branchDec->depth % CORRECTION_WARMSTART == 0
				if (warmstarted) {
		...
		if (iteration == 0 && P->gapMeasure == LB_SHIFT) {
			std::cout << "we update the shift\n";

			// search for ideal point
			P->shiftVec = std::vector<double>(lp->get_p(), 1000000);
			for (std::list<Point*>::iterator y = extrPoints.begin(); y != extrPoints.end(); y++) {
				for (int k = 0; k < lp->get_p(); k++) {
					if ((*y)->get_objVector(k) < P->shiftVec[k]) {
						P->shiftVec[k] = (*y)->get_objVector(k);
					}
				}
			}

			// shift ideal point to origin
			for (int k = 0; k < lp->get_p(); k++) {
				if (P->shiftVec[k] >= 0) P->shiftVec[k] = 0;
				else P->shiftVec[k] = abs(P->shiftVec[k]);
				P->shift += P->shiftVec[k];
			}
			std::cout << " shift: " << P->shift << "\n";
		}
	}
 ...
 * \param U UpperBoundSet. The upper bound set updated.
 */
bool LinearRelaxation::gatherIntegerSolutions(UpperBoundSet& U) {

	std::list<Point*>::iterator vertex;
	int nbPts = 0;
	bool existsIntegerPoints = false;
	bool isNd = false, newNdPtsFound = false;
	...
	for (vertex = extrPoints.begin(); vertex != extrPoints.end(); vertex++) {
		if (!(*vertex)->isOnBoundingBox()) { // if the point is not on the bouding box and is integer     && (*vertex)->isInteger()
			...
			if ((*vertex)->isInteger()) {
				...
				if ((*vertex)->isNew()) {
					isNd = U.updateUB(**vertex);
					if (!newNdPtsFound && isNd) newNdPtsFound = true;
				}
	...
	//if (!newNdPtsFound) std::cout << "iteration " << iteration << " -> no new nd in LB set\n";

	return newNdPtsFound;
}
...
/*! \brief Computes most often fractional value of variable $x_i$ among the extreme points of the linear relaxation.
 * MOFV within VF bounds!
 * 
 * \param slub SLUB. The slub that defines the part of the objective space to search in.
 * \param i int. Index of the splitting variable.
 * \return the index of the most often fractional variable, as an int.
 */
int LinearRelaxation::computeMostOftenFractionalSplittingValueVF(SLUB& slub, int i, int minLb, int maxUb) {
	...
	std::vector<int> nbFracVal(floor(maxVal) + 1, 0);
	//if (iteration == 2022) {
		//for (int i = 0; i < nbFracVal.size(); i++) {
			////std::cout << "fracVal " << i << " : " << nbFracVal[i] << "\n";
		//}
		//std::cout << "nb pts: " << nbPts << "\n";
	//}
	for (pt = extrPoints.begin(); pt != extrPoints.end(); pt++) {
		if (!(*pt)->isOnBoundingBox() && slub.dominated(*pt) && (*pt)->get_nbVar() != 0) {
			val = (*pt)->get_preImage(i);
	...
	for (int j = 0; j < floor(maxVal) + 1; j++) {
		if (j < minLb)
			nbFracVal[j] = -1;
		else if (j > maxUb)
			nbFracVal[j] = -1;
	...
	int vl = minLb;
	if (nbFracVal.size() == 0 || nbPts == 0) {
		//vl = branchDec->lb[i];
		vl = minLb;
		//if (iteration == 2319) std::cout << "no pts\n";
	}
	else {
		vl = std::distance(nbFracVal.begin(), std::max_element(nbFracVal.begin(), nbFracVal.end())); // we get the most occuring value

		if (nbFracVal[vl] <= 0) { // no decimal value => call median rule
			//vl = computeMedianSplittingValue(slub, i, maxUb);
			//vl = floor(avgVal);
			//vl = computeRandomSplittingValue(slub, i, branchDec->ub[i]);
			vl = floor((maxUb + minLb) / 2);
		}
	}
...
void LinearRelaxation::computeFractionalProportion(std::vector<int>* I, SLUB& s) {
	...
	std::vector<int> nbFrac(I->size(), 0); // = std::vector<int>(I->size(), 0); // count how many times each variable is fractional
...
/* Generate cover cuts from each extreme points of the LB set
 *
 * \param bd, branching decisions. Used to get the OB constraints.
 * \param pb, mathematical model. To get the objective costs of the variables when computing the cuts.
 */
void LinearRelaxation::extractCoverCuts(BranchingDecisions* bd, MathematicalModel* pb) {

	//if (iteration == 3689) std::cout << " size : " << bd->cc.size();
	SLUB s = SLUB(bd->slub);
	//std::cout << "\n ---------- new node ------------ \n";
	for (std::list<Point*>::iterator y = extrPoints.begin(); y != extrPoints.end(); y++) {
		if(s.dominated(*y)) {
			//std::cout << "  -> new pts\n";
			//(*y)->generateCoverCuts(bd, pb, iteration); // iteration
			(*y)->generateLCI(bd, pb, iteration); // iteration
		}
	}
...
void LinearRelaxation::checkViolatingCut(BranchingDecisions* bd) {
	...
	for (std::list<Point*>::iterator y = extrPoints.begin(); y != extrPoints.end(); y++) {
		if (!(*y)->is_ray() && sl.dominated(*y)) {
			...
			covered = std::vector<bool>(lp->get_n(), false);
			for (int k = 0; k < lp->get_p(); k++) {
...
double LinearRelaxation::getPercentageViolation(BranchingDecisions* bd) {

	int cptViolate(0), cptTotal(0), i(0);
	bool feas(false);
	...
	for (std::list<Point*>::iterator y = extrPoints.begin(); y != extrPoints.end(); y++) {
		if (!(*y)->is_ray()) {
			...
			feas = true;
			i = 0;
			while (feas && i < lp->get_p()) {
				if ((*y)->get_objVector(i) >= bd->slub[i]) feas = false;
				i++;
			}
			i = 0;
			while (feas && i < lp->get_n()) {
				if ((*y)->get_preImage(i) > bd->ub[i] || (*y)->get_preImage(i) < bd->lb[i]) feas = false;
				i++;
			}
			if (!feas) cptViolate++;
		}
	...
	return 100 * cptViolate / cptTotal;
}

void LinearRelaxation::planReset() {
	reset = true;
}
...
std::list<Hyperplane*>* LinearRelaxation::getListFacets() {
	return &facets;
}
